...
static void dump_nodes( const filter_node_t *node, int level, int skip_tagged, FILE *f ) 
{
	...
	while ( node )
	{
		...
		{
			const char *_opstr = op2str( node->fop );

			if ( node->is_date ) 
			{
				if ( node->fop == FOP_LT ) // do not print default action for dates
					_opstr = "";
				n = sprintf( buf, "date %s\"%s\"", _opstr, node->p2.string );
			}
			else 
			{
				...
				{
					if ( node->is_quoted )
						n = sprintf( buf, "%s %s\"%s\"", node->p1, _opstr, node->p2.string );
					else
						n = sprintf( buf, "%s %s%s", node->p1, _opstr, node->p2.string );
				}
				else 
				{
					n = sprintf( buf, "%s %s%i", node->p1, _opstr, node->p2.integer );
				}
			}
