...
static shader_t *FinishShader( void ) {
	...
	if ( shader.isSky || ( tr.sunShaderName && tr.sunShaderName[0] && Q_stricmp( shader.name, "sun" ) == 0 ) ) {
		if ( shader.isSky ) {
...
void R_FindLightmap( int *lightmapIndex ) {
	...
	image = R_FindImageFile( fileName, IMGFLAG_LIGHTMAP | IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE );
	if ( image == NULL ) {
		*lightmapIndex = LIGHTMAP_BY_VERTEX;
		return;
	}
